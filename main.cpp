...
template<typename T>
std::pair<Vec3<Dual2<T>>, Vec3<Dual2<T>>> compute_phase_factors(
    const Vec3<double>& O1,
    const Vec3<double>& O2,
    const Vec3<Dual2<T>>& H)
{
    Vec3<Dual2<T>> ka, kb;

    ka[0] = -0.5 * (O1[1] * H[2] - O1[2] * H[1]);
    ka[1] = -0.5 * (O1[2] * H[0] - O1[0] * H[2]);
    ka[2] = -0.5 * (O1[0] * H[1] - O1[1] * H[0]);

    kb[0] = +0.5 * (O2[1] * H[2] - O2[2] * H[1]);
    kb[1] = +0.5 * (O2[2] * H[0] - O2[0] * H[2]);
    kb[2] = +0.5 * (O2[0] * H[1] - O2[1] * H[0]);

    return { ka, kb };
}

 //Final test function: real input H[3] → complex-valued output
static std::vector<Dual2<std::complex<double>>> testComplexAnalog(const Dual2<double> H[3], int option) {
    using cplx = std::complex<double>;
    const cplx i(0.0, 1.0);

    // Make a complex-valued Dual2
    Dual2<cplx> Z;
    Z.val = i * (H[0].val + H[1].val + H[2].val);  // mimic phase-like behavior
    Z.d1 = i * (H[0].d1 + H[1].d1 + H[2].d1);
    Z.d2 = i * (H[0].d2 + H[1].d2 + H[2].d2);

    // Do something complex-valued
    auto res = exp(Z);  // like computing exp(i·k·r)

    std::vector<Dual2<cplx>> out(3);
    for (int mu = 0; mu < 3; ++mu) {
        out[mu].val = res.val * H[mu].val;
        out[mu].d1 = res.val * H[mu].d1 + res.d1 * H[mu].val;
        out[mu].d2 = res.val * H[mu].d2 + 2.0 * res.d1 * H[mu].d1 + res.d2 * H[mu].val;
    }
...
static void test_dual2_operators(bool verbose = false) {
    double s1 = 2.0;
    std::complex<double> c1(1.0, 2.0);
    Dual2<double> a(2.0, 1.0, 0.5); // a = 2, da/dx = 1, d^2a/dx^2 = 0.5
    Dual2<double> b(3.0, 0.5, 0.25); // b = 3, db/dx = 0.5, d^2b/dx^2 = 0.25
    if (verbose) {
        std::cout << "\nTesting left and right hand side operators with duals:\n";
        std::cout << "a + b: " << (a + b) << "\n";
        std::cout << "a - b: " << (a - b) << "\n";
    ...
    {
		// If verbose is false, we can skip printing the results and just perform the operations
        Dual2<double> result1 = a + b;
        Dual2<double> result2 = a - b;
        Dual2<double> result3 = a * b;
        Dual2<double> result4 = a / b;
        Dual2<double> result5 = a + s1;
        Dual2<double> result6 = a - s1;
        Dual2<double> result7 = a * s1;
        Dual2<double> result8 = a / s1;
        Dual2<double> result9 = s1 + a;
        Dual2<double> result10 = s1 - a;
        Dual2<double> result11 = s1 * a;
        Dual2<double> result12 = s1 / a;
        auto result13 = a + c1;
        auto result14 = a - c1;
        auto result15 = a * c1;
        auto result16 = a / c1;
		...
		std::cout << "Completed Dual2 non-complex operations successfully.\n";
    }
    
}

static void test_dual2_operators_complex(bool verbose = false) {
    Dual2<std::complex<double>> a(2.0, 1.0, 0.5); // a = 2, da/dx = 1, d^2a/dx^2 = 0.5
    Dual2<std::complex<double>> b(3.0, 0.5, 0.25); // b = 3, db/dx = 0.5, d^2b/dx^2 = 0.25
    std::complex<double> c1(1.0, 2.0);
    if (verbose) {
        std::cout << "\nTesting left and right hand side operators with complex duals:\n";
        std::cout << "a + b: " << (a + b) << "\n";
    ...
    {
		// If verbose is false, we can skip printing the results and just perform the operations
        Dual2<std::complex<double>> result1 = a + b;
        Dual2<std::complex<double>> result2 = a - b;
        Dual2<std::complex<double>> result3 = a * b;
        Dual2<std::complex<double>> result4 = a / b;
        Dual2<std::complex<double>> result5 = a + 2.0;
        Dual2<std::complex<double>> result6 = a - 2.0;
        Dual2<std::complex<double>> result7 = a * 2.0;
        Dual2<std::complex<double>> result8 = a / 2.0;
        Dual2<std::complex<double>> result9 = 2.0 + a;
        Dual2<std::complex<double>> result10 = 2.0 - a;
        Dual2<std::complex<double>> result11 = 2.0 * a;
        Dual2<std::complex<double>> result12 = 2.0 / a;
        Dual2<std::complex<double>> result13 = a + c1;
        Dual2<std::complex<double>> result14 = a - c1;
        Dual2<std::complex<double>> result15 = a * c1;
        Dual2<std::complex<double>> result16 = a / c1;
        Dual2<std::complex<double>> result17 = c1 + a;
        Dual2<std::complex<double>> result18 = c1 - a;
        Dual2<std::complex<double>> result19 = c1 * a;
		Dual2<std::complex<double>> result20 = c1 / a;

		std::cout << "Completed Dual2 complex operations successfully.\n";
    }
}
...
static void test_higher_order_polynomial_dual2() {
    std::cout << "Second-order Dual2 example:\n";
    std::cout << "x: f(x) = x^3 + x^2 + x + 1\n";
    bool all_match = true;
    for (double i = 0; i <= 5; i += 0.1) {
        // Analytical solution: f(x) = x^3 + x^2 + x + 1
        double f_x = pow(i, 3) + pow(i, 2) + i + 1.0;
        double f_dx = 3 * pow(i, 2) + 2 * i + 1.0; // First derivative
        double f_d2x = 6 * i + 2.0; // Second derivative

        Dual2<double> x3(i, 1.0, 0); // x = i, dx/dx = 1, d^2x/dx^2 = 0
        Dual2<double> y3 = pow(x3, 3) + pow(x3, 2) + x3 + 1.0;

        auto almost_equal = [](double a, double b, double eps = 1e-9) {
            return std::abs(a - b) < eps;
        };
        if (!(almost_equal(y3.val, f_x) &&
              almost_equal(y3.d1, f_dx) &&
              almost_equal(y3.d2, f_d2x))) {
            all_match = false;
            std::cout << "x: " << x3.val << "\n";
            std::cout << "  Mismatch in values!\n";
            std::cout << "  Expected f(x) = " << f_x << ", df/dx = " << f_dx << ", d^2f/dx^2 = " << f_d2x << "\n";
            std::cout << "  Got f(x) =      " << y3.val << ", df/dx = " << y3.d1 << ", d^2f/dx^2 = " << y3.d2 << "\n\n";
        }
    }
    if (all_match) {
...
int main() {
	// Example for second-order Dual2 with higher order polynomial (x^3 +x^2 + x +1)
	test_higher_order_polynomial_dual2();
    ...
    // Test the Dual2 class with non-complex numbers
    std::cout << "\nTesting non-complex Dual2 operators:\n";
    test_dual2_operators();

    // Test the Dual2 class with complex numbers
    std::cout << "\nTesting complex Dual2 operators:\n";
    test_dual2_operators();
